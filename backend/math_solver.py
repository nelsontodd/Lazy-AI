...
if __name__ == '__main__':
    hwdir, hw_file_name = "hw/", "poorly_formatted_math"
    hw_mmd = utils.mathpix_pdf_to_mmd(hw_file_name, pdf_id="2023_04_10_88d52bf6ed7bb2cb3e1eg")
    print("""Inputting {} tokens into
            GPT-4.""".format(utils.num_tokens_from_messages(constants.MATH_HW +
                hw_mmd)))
    solutions = utils.promptGPT(constants.MATH_HW, hw_mmd, "gpt-4")
    utils.to_md(solutions, hw_file_name)
    utils.pandoc_pdf("outputs/{}".format(hw_file_name),
            "outputs/{}".format("hw_answer_sheet_temp"))
    print("Prompted gpt-4.")

    reportlab_temp_pdf="outputs/{}.pdf".format("hw_reportlab_temp")
    ...
    pandoc_pdf = PdfReader("outputs/{}.pdf".format("hw_answer_sheet_temp"))
    ...
    with open("outputs/{}.pdf".format(hw_file_name), "wb") as merged_file:
